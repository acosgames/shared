...
module.exports = { validateSimple, validate, validateField };
...
const validationRules = {
    ...
    min: (fields, table, rules, value, errors) => {
        if (typeof value === "number") {
            if (value < rules.min) {
                errors.push(
                    ValidateError("E_NUMBER_TOOSMALL", [rules.label, rules.min])
                );
            }
        } else if (typeof value === "string") {
            if (value.length < rules.min) {
                errors.push(
                    ValidateError("E_FIELD_TOOSHORT", [rules.label, rules.min])
                );
            }
        }
    },
    max: (fields, table, rules, value, errors) => {
        if (typeof value === "number") {
            if (value > rules.max) {
                errors.push(
                    ValidateError("E_NUMBER_TOOBIG", [rules.label, rules.max])
                );
            }
        }
        if (typeof value === "string") {
            if (value.length > rules.max) {
                errors.push(
                    ValidateError("E_FIELD_TOOLONG", [rules.label, rules.max])
                );
            }
        }
    },
