...
// import { muRating, muDefault, sigmaDefault  } from "../util/ratingconfig.js";
...
class AchievementService {
    ...
    async getAchievementDefinitions(game_slug, stats) {
        try {
            ...
            response = await db.sql(`SELECT ad.*
    ...
    async claimAchievement(game_slug, shortid, achievement_slug) {
        try {
            let db = await mysql.db();
            let { results } = await db.update("person_achievement", {
                claimed: 1,
            }, "achievement_slug = ? AND game_slug = ? AND shortid = ? AND completed IS NOT NULL", [achievement_slug, game_slug, shortid]);
            if (results.affectedRows != 1 || results.changedRows != 1)
                throw new Error("Achievement already claimed.");
            let achievements = await db.query(`SELECT * FROM achievement_definition WHERE achievement_slug = ? AND game_slug = ?`, [achievement_slug, game_slug]);
    ...
    async updatePlayerAchievements(meta, gamestate) {
        try {
            ...
            let statDefs = await db.query(`SELECT *
            ...
            let achievementDefs = await db.query(`SELECT *
            ...
            try {
                let achievements = await db.query(`SELECT *
                    FROM person_achievement
                    WHERE game_slug = ?
                    and shortid in (?)`, [game_slug, shortids]);
            ...
            for (let shortid of shortids) {
                ...
                player.stats["ACOS_PLAYTIME"] = Math.floor((gamestate.room.endtime - gamestate.room.starttime) / 1000);
                ...
                for (let achievement of achievementDefs) {
                    let pAchievement = (playerAchievements[shortid] &&
                        playerAchievements[shortid][achievement.achievement_slug]) ||
                        {};
                    //already completed, skip
                    if (pAchievement.completed)
                        continue;
                    //increment stat 1
                    if (achievement.stat_slug1) {
                        this.updateAchievementStat(1, achievement, statMap, pAchievement, player.stats);
                    }
                    else {
                    ...
                    if (achievement.stat_slug2) {
                        this.updateAchievementStat(2, achievement, statMap, pAchievement, player.stats);
                    }
                    else {
                    ...
                    if (achievement.stat_slug3) {
                        this.updateAchievementStat(3, achievement, statMap, pAchievement, player.stats);
                    }
                    else {
                    ...
                    try {
                        //calculate progress
                        let { value, maxValue, percent } = this.calculateAchievementProgress(achievement, pAchievement);
            ...
            if (updateAchievements?.length > 0) {
                let matchInsertResults = await db.insertBatch("person_achievement", updateAchievements, ["achievement_slug", "game_slug", "shortid"], [], ["tsupdate", "tsinsert"]);
            }
            let playerList = {};
            for (let shortid of shortids) {
                playerList[shortid] = {};
    ...
    calculateAchievementProgress(achievement, progress) {
        let { stat_slug1, goal1_valueTYPE, goal1_valueINT, goal1_valueFLOAT, 
        // goal1_valueSTRING,
        stat_slug2, goal2_valueTYPE, goal2_valueINT, goal2_valueFLOAT, 
        // goal2_valueSTRING,
        stat_slug3, goal3_valueTYPE, goal3_valueINT, goal3_valueFLOAT, 
        // goal3_valueSTRING,
        all_required, times_in_a_row, } = achievement;
        let { stat1_valueINT, stat1_valueFLOAT, 
        // stat1_valueSTRING,
        ...
        // }
        if (stat_slug1) {
            let stat1progress = this.calculateStatProgress(stat_slug1, goal1_valueTYPE, goal1_valueINT, goal1_valueFLOAT, 
            // goal1_valueSTRING,
            stat1_valueINT, stat1_valueFLOAT, 
            // stat1_valueSTRING,
            times_in_a_row, played);
            if (stat1progress)
                status.push(stat1progress);
        }
        if (stat_slug2) {
            let stat2progress = this.calculateStatProgress(stat_slug2, goal2_valueTYPE, goal2_valueINT, goal2_valueFLOAT, 
            // goal2_valueSTRING,
            stat2_valueINT, stat2_valueFLOAT, 
            // stat2_valueSTRING,
            times_in_a_row, played);
            if (stat2progress)
                status.push(stat2progress);
        }
        if (stat_slug3) {
            let stat3progress = this.calculateStatProgress(stat_slug3, goal3_valueTYPE, goal3_valueINT, goal3_valueFLOAT, 
            // goal3_valueSTRING,
            stat3_valueINT, stat3_valueFLOAT, 
            // stat3_valueSTRING,
            times_in_a_row, played);
            if (stat3progress)
                status.push(stat3progress);
        }
        let value = 0;
        let maxValue = 0;
        let percent = 0;
        if (!all_required) {
            ...
            value = status.reduce((total, curr) => curr === false ? total : total + Math.min(curr.value, curr.maxValue), 0);
            //use the largest max goal of the stats
            maxValue = status.reduce((total, curr) => (curr.maxValue >= total ? curr.maxValue : total), 0);
            ...
            value = status.reduce((total, curr) => curr === false ? total : total + Math.min(curr.value, curr.maxValue), 0);
            //use the largest max goal of the stats
            maxValue = status.reduce((total, curr) => (curr === false ? total : total + curr.maxValue), 0);
            if (Number.isNaN(value))
                value = 0;
            if (Number.isNaN(maxValue))
                maxValue = 1;
    ...
    calculateStatProgress(stat_slug, goal_valueTYPE, goal_valueINT, goal_valueFLOAT, 
    // goal_valueSTRING,
    stat_valueINT, stat_valueFLOAT, 
    // stat_valueSTRING,
    times_in_a_row, played) {
        //no negative numbers
