...
class LeaderboardService {
    ...
    createRedisKey(config) {
        ...
        if (config?.game_slug)
            key.push(config.game_slug);
        if (config?.countrycode)
            key.push(config.countrycode);
        if (typeof config?.season === "number")
            key.push("S" + config.season);
        if (config?.stat_slug)
            key.push(config.stat_slug);
        if (config?.division_id)
            key.push(config.division_id);
        if (config?.monthly) {
            let now = new Date();
    ...
    async getRankLeaderboard(config) {
        ...
        if (rankings.length >= 100)
            total = await this.getRedisLeaderboardCount(config);
    ...
    async getHighscoreLeaderboard(config) {
        ...
        if (!config.aggregate && config?.monthly) {
            ...
            if (config?.countrycode)
                value.push(config.countrycode);
            if (!config?.monthly && typeof config?.season === "number")
                value.push(config.season);
    ...
    async getStatLeaderboard(config) {
        ...
        if (!statDef)
            return [];
        let valueType = "INT";
        if (statDef.valueTYPE == 1 || statDef.valueTYPE == 2)
            valueType = "FLOAT";
    ...
    async getAllRanks(config, onResults) {
        if (!config || !config?.game_slug || !config?.season)
            return;
        let db = await mysql.db();
        let response;
    ...
    async rankLeaderboard(game_slug, shortid, displayname, config) {
        try {
            ...
            if (displayname) {
                ...
                game.localboard =
                    (await this.getPlayerGameLeaderboard(game_slug, displayname, playerRank, config)) || [];
            }
            else {
                game.localboard = [];
            }
    ...
    async findGameRankNational(game_slug, shortid, displayname, countrycode) {
        try {
            ...
            if (displayname) {
                ...
                game.localboard =
                    (await this.getPlayerGameLeaderboard(game_slug, displayname, playerRank, countrycode)) || [];
            }
            else {
                game.localboard = [];
            }
    ...
    async findGameLeaderboardHighscore(game_slug, shortid, displayname) {
        try {
            ...
            if (displayname) {
                let playerRank = await this.getPlayerGameHighscore(game_slug, displayname, countrycode);
                game.localboard =
                    (await this.getPlayerGameLeaderboardHighscore(game_slug, displayname, playerRank, countrycode)) || [];
            }
            else {
                game.localboard = [];
            }
