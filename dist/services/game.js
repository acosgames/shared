...
class GameService {
    ...
    async findGame(game_slug, ignoreExtra = false) {
        try {
    ...
    async findGamePerson(game_slug, shortid, displayname) {
        try {
            ...
            let response = await db.sql(`SELECT 
            ...
            WHERE pr.shortid = ? AND pr.game_slug = ? and pr.season = ?`, [shortid, game_slug, game.season]);
            if (response.results && response.results.length == 0) {
            ...
            game.stats = await stats.getGameStats(game_slug, game.maxplayers == 1);
            game.achievements = await achievements.getAchievementProgress(game_slug, shortid, game.stats);
            let cleaned = {
                game: {
