...
import { genUnique64string, generateAPIKEY } from "../util/idgen.js";
import { GeneralError, CodeError, SQLError } from "../util/errorhandler.js";
import { validateSimple, } from "../util/validation.js";
...
export default class DevGameService {
        ...
        try {
            ...
            if (game.gameid) {
                ...
                response = await db.sql(`
                    ...
                    LEFT JOIN person p ON b.ownerid = p.id
    ...
    async createGameVersion(db, gameVersion) {
        try {
            db = db || (await mysql.db());
            // let gameVersion = {
            //     gameid: {
            ...
            //     css: hasCSS ? 1 : 0,
            // };
            let maxVersion = await db.query("SELECT MAX(version) as maxVersion FROM game_version WHERE gameid = ?", [gameVersion.gameid]);
    ...
    async updateStats(db, gameFull, apiKey, gameSettings) {
        try {
            ...
            if (!stats)
                throw new GeneralError("E_MISSING_STATS");
            let existingStatDefs = await db.sql(`SELECT * FROM stat_definition WHERE game_slug = ? AND isactive = 1`, [gameFull.game_slug]);
            let existingMap = {};
            ...
            for (let abbr in stats) {
                ...
                try {
                    if (stat_slug in existingMap) {
                        ...
                        let result = await db.update("stat_definition", row, "game_slug=? AND stat_slug=?", [gameFull.game_slug, stat_slug], ["tsupdate"]);
                    }
                    else {
            ...
            for (let stat_slug in existingMap) {
                ...
                if (stat.updated)
                    continue;
                try {
                    let result = await db.update("stat_definition", { isactive: 0 }, "game_slug=? AND stat_slug=?", [gameFull.game_slug, stat.stat_slug]);
                }
                catch (e2) {
                    console.error(e2);
                    return false;
    ...
    async updateAchievement(game, achievement, user, db) {
        ...
        try {
            ...
            let existResponse = await db.sql(`SELECT * FROM achievement_definition WHERE game_slug = ? AND achievement_slug = ?`, [achievement.game_slug, achievement.achievement_slug]);
            let dbresult;
            if (existResponse?.results?.length > 0) {
                let { results } = await db.update("achievement_definition", achievement, "game_slug=? AND achievement_slug=?", [achievement.game_slug, achievement.achievement_slug]);
                dbresult = results;
            }
            else {
    ...
    async inviteToGithub(user) {
        ...
        try {
            ...
            console.error(e3);
            if (e3?.response?.data?.errors &&
                e3.response.data.errors[0]?.message.indexOf("Invitee is already a part of this organization") > -1) {
                return true;
            }
        }
        return false;
...
;
//# sourceMappingURL=devgame.js.map