...
import { uniqueName } from "../util/utils.js";
...
class RatingService {
    ...
    async createPersonRank(player, game_slug) {
        try {
            ...
            if (!player)
                throw new Error("Player does not exist: " + JSON.stringify(player));
    ...
    async findPlayerRating(shortid, meta, game_slug) {
        try {
            ...
            response = await db.sql(`SELECT 
                    ...
                    WHERE b.shortid = ?`, [game_slug, meta?.season || 0, shortid]);
    ...
    async updateLeaderboardHighscore(game_slug, players) {
        try {
            ...
            for (var id in players) {
                let player = players[id];
                if (!player?.displayname || typeof player?.highscore === "undefined")
                    continue;
                members.push({
                    value: player.displayname,
    ...
    async updatePlayerDivision(shortid, game_slug, season, divison_id) {
        try {
            ...
            let db = await mysql.db();
            var response = await db.update("person_rank", update, "shortid = ? AND game_slug = ? AND season = ?", [shortid, game_slug, season]);
            if (response && response.results.affectedRows > 0) {
                return true;
            }
        }
