...
import { SQLError } from "../util/errorhandler.js";
var pools = {};
    ...
    async db() {
        try {
            ...
            return {
                ...
                increment: (table, row, where, whereValues) => {
                    ...
                    return new Promise((resolve, reject) => {
                        try {
                            ...
                            var query = this.pool.query("UPDATE " +
                                table +
                                " SET " +
                                keys.join(",") +
                                " " +
                                where, values, function (error, results, fields) {
                                if (error) {
                ...
                insertBatch: (table, rows, primarykeys, incrementKeys, ignoreKeys) => {
                    // if( rows.length > 1000 ) {
                    ...
                    return new Promise((resolve, reject) => {
                        try {
                            ...
                            var query = this.pool.query(sql, valuesList, function (error, results, fields) {
                                if (error) {
                                    // conn.rollback();
                                    // conn.release();
                                    console.error(error);
