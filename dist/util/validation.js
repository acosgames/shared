...
const validationRules = {
    ...
    required: (fields, table, rules, value, errors) => {
        ...
        if (typeof value === "undefined" ||
            ...
            errors.push(ValidateError("E_FIELD_REQUIRED", rules.label + " is required."));
        }
        else if (rules.type && typeof value !== rules.type) {
            errors.push(ValidateError("E_FIELD_INVALIDTYPE", rules.label + " has invalid type."));
        }
    },
    integer: (fields, table, rules, value, errors) => {
        if (typeof value === "string") {
            if (Number.isNaN(Number.parseInt(value, 10))) {
                errors.push(ValidateError("E_NOT_INTEGER", rules.label + " is a string, must be an integer."));
            }
            return;
        }
        if (!Number.isInteger(value)) {
            errors.push(ValidateError("E_NOT_INTEGER", rules.label + " must be an integer."));
        }
    },
    min: (fields, table, rules, value, errors) => {
        if (typeof value === "number") {
            if (value < rules.min) {
                errors.push(ValidateError("E_NUMBER_TOOSMALL", rules.label + " is less than min, " + rules.min));
            }
        }
        else if (typeof value === "string") {
            if (value.length < rules.min) {
                errors.push(ValidateError("E_FIELD_TOOSHORT", rules.label +
                    " is too short, min of " +
                    rules.min +
                    " characters."));
            }
    ...
    max: (fields, table, rules, value, errors) => {
        if (typeof value === "number") {
            if (value > rules.max) {
                errors.push(ValidateError("E_NUMBER_TOOBIG", rules.label + " is more than max, " + rules.max));
            }
        }
        if (typeof value === "string") {
            if (value.length > rules.max) {
                errors.push(ValidateError("E_FIELD_TOOLONG", rules.label +
                    " is too long, max of " +
                    rules.max +
                    " characters."));
            }
    ...
    greaterthanequal: (fields, table, rules, value, errors) => {
        ...
        if (otherRules.type != "number") {
            errors.push(ValidateError("E_FIELD_INCOMPATIBLE", [
                rules.label,
                otherRules.label,
                "number",
            ]));
