...
import credutil from '../util/credentials.js';import { utcDATETIME  } from '../util/datefns.js';
...
export default class ObjectStorage {
    ...
    upload(params, options?: any, cb?: any) {
        if (!params) {
            console.error("S3 Upload Failed, missing params.");
            return;
        }
    ...
    async multiPartUpload(Bucket, Key, buffer, options?: any) {
    ...
    downloadServerScript(Key) {
        ...
        return new Promise(async (rs, rj) => {
            try {
                ...
                console.log('Checking if Server Script exists at path: ', localPath);
                let fileExists = false;
                try {
                ...
                if (fileExists) {
                    let data = fs.readFileSync(localPath, 'utf8');
