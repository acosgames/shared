...
import { generateAPIKEY  } from '../util/idgen.js';
import NodeCache from "node-cache";
const cache = new NodeCache({ stdTTL: 60, checkperiod: 120 });
const maxNacks = 10;

import credutil from '../util/credentials.js';
import ServerRemoteService from './instanceremote.js';
...
class RabbitMQService {
    ...
    async subscribe(exchange, pattern, callback, queue) {
        ...
        try {
            ...
            let queueAlias = queue;
            if( !queue ) {
                queueAlias = await this.findExistingQueue(queue);
                if (!queueAlias) {
                    console.error("Failed to find existing queue for ", queue);
                    return false;
                }
                this.subscribeQueue(queueAlias, callback); 
            }
