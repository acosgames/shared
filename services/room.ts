...
class RoomService {
    ...
    async assignPlayersToRoom(shortids: string[], room_slug: string, game_slug: string) {
        try {
    ...
    async assignPlayerRoom(shortid: string, room_slug: string, game_slug: string) {
        try {
            let db = await mysql.db();
    ...
    async removePlayerRoom(shortid: string, room_slug: string) {
        try {
            let db = await mysql.db();
    ...
    async updateRoomPlayerCount(room_slug: string, player_count: number) {
        try {
            // let db = await mysql.db();
            // let update = {
            //     player_count
    ...
    async findPlayerRoom(shortid: string, game_slug: string) :Promise<PlayerGameRoom[]> {
        try {
            let db = await mysql.db();
            var response;
    ...
    async addError(game_slug: string, version: string, error: { type: string; title: string; body: string }) {
        let row = {
            game_slug,
            version,
            type: error.type,
    ...
    async findPlayerRooms(shortid: string) : Promise<PlayerGameRoomExtended[]> {
        try {
            let db = await mysql.db();
            var response;
    ...
    async findRoom(room_slug): Promise<RoomMeta> {
        try {
    ...
    async findRooms(game_slug) {
        try {
            ...
            response = await db.sql(
                ...
                WHERE r.game_slug = ? 
                AND r.status = 0 
                ORDER BY r.version desc, r.rating desc`,
                [game_slug]
            );
            return response.results;
        } catch (e) {
