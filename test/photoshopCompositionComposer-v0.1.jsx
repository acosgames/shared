...
function combine() {
  var artLayerCollectionCollection = getArtLayerCollectionCollection(),
    artLayerCollectionCollectionCombinations = getCombinations(
      artLayerCollectionCollection,
      getLayerSetsCount()
    ),
    continueConfirmation;

  if (!artLayerCollectionCollectionCombinations.length)
    return alert(
      "Script has aborted. No combinations found. Please make sure no empty groups are present."
    );

  continueConfirmation = confirm(
    artLayerCollectionCollectionCombinations.length +
      " combinations found. Would you like to continue?"
  );
  ...
  var includePSDFiles = confirm(
    "Would you like to include corresponding PSD documents?"
  );
  ...
  for (var i = 0; i < artLayerCollectionCollectionCombinations.length; i++) {
    ...
    for (
      var z = 0;
      z < artLayerCollectionCollectionCombinations[i].length;
      z++
    ) {
    ...
    saveDocumentAsPNG(
      savePath +
        "/" +
        normalizeSaveFileName(artLayerNames.join("")).substr(0, 254)
    );
    if (includePSDFiles)
      saveDocumentAsPSD(
        savePath +
          ...
          normalizeSaveFileName(
            artLayer.parent.name + artLayerNames.join("")
          ).substr(0, 254)
      );
  }
}
